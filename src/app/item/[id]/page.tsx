...
export default function ItemDetailPage({ params }: ItemPageProps) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => setProgressSuccess(null), 2500);
  ...
  async function handleIncrement() {
    if (!item) return;
    if (!primary) {
      setProgressError("尚未設定主進度，請先新增並設為主進度。");
      return;
    }
    setProgressError(null);
    ...
    try {
      ...
      await batch.commit();
      setProgressSuccess("已更新主進度");
    } catch (err) {
      console.error("更新主進度時發生錯誤", err);
      setProgressError("更新主進度時發生錯誤");
    } finally {
        ...
        <section className="space-y-6 rounded-2xl border bg-white/70 p-6 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="space-y-1">
              ...
              <p className="text-sm text-gray-500">檢視主進度並可透過下方快速 +1 更新。</p>
            </div>
            <button
              type="button"
